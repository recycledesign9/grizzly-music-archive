...
(function () {
  ...
  function postJSON(url, data, callback) {
    ...
    .then(callback)
    .catch(function (e) { callback({ ok: false, message: e.message }); });
  }
  ...
  function openBrowser(path) {
    ...
    .then(function (data) {
      ...
      var dirs = (data.dirs || []).filter(function (d) { return !d.up; });
      if (dirs.length === 0) {
      ...
      for (var ei = 0; ei < entries.length; ei++) {
        ...
        entries[ei].addEventListener('mouseenter', function () { this.classList.add('bg-body-secondary'); });
        entries[ei].addEventListener('mouseleave', function () { this.classList.remove('bg-body-secondary'); });
      }
